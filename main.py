...
def train_one_epoch(epoch, dataloader, model, criterion, optimizer, device,
    ...
    imgs_res_folder = os.path.join(args.output_dir, 'current_res')  # outputs/current_res
    ...
    for batch_id, sample_batched in enumerate(dataloader):
        ...
        loss.backward()
        optimizer.step()
        loss_avg.append(loss.item())    # .item() Returns the value of this tensor as a standard Python number. This only works for tensors with one element.
...
def parse_args():
    ...
    parser.add_argument('--choose_test_data',
                        type=int,
                        default=9,   # CLASSIC = 9 (for custom data)
                        help='Already set the dataset for testing choice: 0 - 9')
    # ----------- test -------0--


    TEST_DATA = DATASET_NAMES[parser.parse_args().choose_test_data] # max 9
    test_inf = dataset_info(TEST_DATA, is_linux=IS_LINUX)
    test_dir = test_inf['data_dir']
    is_testing = False # current test _bdcnlossNew256-sd7-1.10.4p5

    # Training settings
    TRAIN_DATA = DATASET_NAMES[9] # BIPED=0, CLASSIC=9
    train_inf = dataset_info(TRAIN_DATA, is_linux=IS_LINUX)
    train_dir = train_inf['data_dir']   # data/magnet_ref_annotation
    ...
    parser.add_argument('--epochs',
                        ...
                        default=1000,
                        metavar='N',
                        help='Number of training epochs (default: 34).')
    ...
    parser.add_argument('--img_width',
                        type=int,
                        default=368,
                        help='Image width for training.') # BIPED 400 BSDS 352/320 MDBD 480, custom 4032 too large
    parser.add_argument('--img_height',
                        type=int,
                        default=240,
                        help='Image height for training.') # BIPED 480 BSDS 352/320, custom 3024 too large
...
def main(args):
    ...
    print("Is this Linux: ", IS_LINUX)

    # Tensorboard summary writer

    tb_writer = None
    training_dir = os.path.join(args.output_dir,args.train_data)    # writting the log to outputs/CLASSIC
    ...
    # model = nn.DataParallel(model)    # This is for using multiple GPUs
    ini_epoch =0
    if not args.is_testing:
        ...
        dataset_train = BipedDataset(args.input_dir,    # data/magnet_ref_annotation
                                     img_width=args.img_width,  # 368
                                     img_height=args.img_height,    # 240
    ...
    dataset_val = TestDataset(args.input_val_dir,   # data/magnet_ref_annotation
                              test_data=args.test_data,     # CLASSIC
                              img_width=args.test_img_width,    # 368
                              img_height=args.test_img_height,  #240
                              mean_bgr=args.mean_pixel_values[0:3] if len(
                                  args.mean_pixel_values) == 4 else args.mean_pixel_values,
    ...
    for epoch in range(ini_epoch,args.epochs):
        ...
        output_dir_epoch = os.path.join(args.output_dir,args.train_data, str(epoch))    # outputs/CLASSIC/epoch
        img_test_dir = os.path.join(output_dir_epoch, args.test_data + '_res')  # outputs/CLASSIC/epoch/CLASSIC_res
        ...
        # Save model after end of every 10 epoch
        if epoch%10==9:
            torch.save(model.module.state_dict() if hasattr(model, "module") else model.state_dict(),
                       os.path.join(output_dir_epoch, '{0}_model.pth'.format(epoch)))   # outputs/CLASSIC/epoch/epoch_model.pth

        # Save the best model
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.module.state_dict() if hasattr(model, "module") else model.state_dict(),
                       os.path.join("outputs/CLASSIC", 'best_model.pth'))   # outputs/CLASSIC/epoch/epoch_model.pth
