    ...
    if is_linux:
        ...
        config = {
            ...
            'BIPED': {
                ...
                'test_list': 'test_pair.lst',
                'train_list': 'train_pair.lst',
                'data_dir': 'data/BIPED/edges',  # mean_rgb
                'yita': 0.5
            },
            'CLASSIC': {
                'img_height': 240,   # 512
                ...
                'test_list': None,
                'train_list': 'train_pair.lst',
                'data_dir': 'data/magnet_ref',  # mean_rgb
                'yita': 0.5          # useless, can be removed
            },
            'DCD': {
...
class TestDataset(Dataset):
    ...
    def _build_index(self):
        ...
        if self.test_data == "CLASSIC":
            ...
            img_dir = os.path.join(self.data_root, "imgs", "test")  # data/magnet_ref_annotation/imgs/test
...
class BipedDataset(Dataset):
    ...
    def _build_index(self):
        ...
        data_root = os.path.abspath(self.data_root)     # data/magnet_ref_annotation
        sample_indices = []
        if self.arg.train_data.lower()=='biped':
            ...
            file_path = os.path.join(data_root, self.arg.train_list)    # data/magnet_ref_annotation/train_pair.lst
            if self.arg.train_data.lower()=='bsds':
            ...
            else:    # For custom dataset, self.arg.train_data = CLASSIC
                with open(file_path, 'r') as f:
                    files = f.readlines()
                files = [line.strip() for line in files]
                pairs = [line.split() for line in files]
                for pair in pairs:
                    ...
                    tmp_gt = pair[1]
                    sample_indices.append(
                        (os.path.join(data_root,tmp_img),       # data/magnet_ref_annotation/xxx.jpg
                         os.path.join(data_root,tmp_gt),))      # data/magnet_ref_annotation/xxx.png
    ...
    def transform(self, img, gt):
        ...
        crop_size = self.img_height if self.img_height == self.img_width else 240  # custom_dataset=1008 MDBD=480 BIPED=480/352 BSDS=320
        ...
        # MODIFIED 2021/11/03: Do not resize to 240x240 Using the original image(368x240)
        # else:
        #     # New addidings
        #     img = cv2.resize(img, dsize=(crop_size, crop_size))
        #     gt = cv2.resize(gt, dsize=(crop_size, crop_size))
        ...
        img = img.transpose((2, 0, 1))      # do HWC->CHW transformation
        img = torch.from_numpy(img.copy()).float()
        gt = torch.from_numpy(np.array([gt])).float()
        return img, gt
